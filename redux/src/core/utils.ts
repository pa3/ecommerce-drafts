...
export function assignDeep<T>(dest: T, change: DeepPartial<T>): T {
  ...
  return Object.fromEntries(
    allKeys.map((key) => {
      ...
      const newValue = change[key as keyof T] as unknown as DeepPartial<T[keyof T]>;
      if (newValue) {
        const nextValue = Array.isArray(oldValue) || typeof oldValue !== "object" ? newValue : assignDeep(oldValue, newValue);
